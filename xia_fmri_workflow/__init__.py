...
def factor_condition_generator(
    ...
    Build a bunch for make a design matrix for next analysis. This bunch is
    for describing the relationship between each onset and parameter.

    Args:
        single_sub_data: A pandas DataFrame which contains data for one subject.
                         It must contains the information about run, onsets, and
                         parameters.
        factors_name: A list of names of conditions which you want to analysis.
        duration: The duration of a TR.

    Returns:
        subject_info: A list of bunch type which can be resolve by SpecifySPMModel
        interface in nipype.
    """
...
def workflow_param_glm_1stlevel(
    root_dir,
    sub_num,
    session_num,
    params_name,
    ...
    if output_dir.is_dir():
        ...
        print(
            "Save subject " + str(sub_num) + "'s data to \n" + str(output_dir)
        )
    ...
    (
        ...
        single_sub_data,
        sub_name,
    ) = nii_selector(root_dir, sub_num, session_num, all_data)
...
def workflow_condition_glm_1stlevel(
    root_dir,
    sub_num,
    session_num,
    factors_name,
